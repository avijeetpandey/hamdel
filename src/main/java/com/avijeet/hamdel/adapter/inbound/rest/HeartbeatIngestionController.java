...
public class HeartbeatIngestionController {
    ...
    public ResponseEntity<Void> ingestProto(@RequestBody byte[] body) throws Exception {
        ...
        HeartbeatEventProto event = mapFromProto(proto);
        ingestHeartbeatUseCase.ingest(event);
        recordLatency(start);
    ...
    public ResponseEntity<Void> ingestJson(@Valid @RequestBody HeartbeatRequest request) {
        long start = System.nanoTime();
        HeartbeatEventProto event = mapFromRequest(request);
        ingestHeartbeatUseCase.ingest(event);
        recordLatency(start);
        return ResponseEntity.accepted().build();
    }
    ...
    private HeartbeatEventProto mapFromProto(com.avijeet.hamdel.proto.HeartbeatEvent proto) {
    ...
    private HeartbeatEventProto mapFromRequest(HeartbeatRequest req) {
        return HeartbeatEventProto.builder()
                .eventId(req.eventId())
